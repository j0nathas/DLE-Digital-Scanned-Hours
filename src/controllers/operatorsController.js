...
exports.getAllOperators = async (req, res) => {
    try {
        ...
        const result = await pool.request().query('SELECT * FROM dbo.Operators');
...
exports.getOperatorsById = async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query('SELECT * FROM dbo.Operators WHERE id = @id');

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Operators não encontrado(a).' });
        }
...
// CREATE NEW
exports.createOperators = async (req, res) => {
    try {
        const data = req.body;
        const pool = await poolPromise();
    ...
    "idType": "int"
        ...
        const query = `INSERT INTO dbo.Operators (${columns.join(', ')}) OUTPUT INSERTED.* VALUES (${variables.join(', ')});`;
...
exports.updateOperators = async (req, res) => {
    ...
    "idType": "int"
        ...
        const query = `UPDATE dbo.Operators SET ${setClauses.join(', ')} OUTPUT INSERTED.* WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Operators não encontrado(a) para atualização.' });
        }
        res.status(200).json(result.recordset[0]);
...
exports.deleteOperators = async (req, res) => {
    try {
        ...
        const pool = await poolPromise();
        const request = pool.request();
        const query = `DELETE FROM dbo.Operators OUTPUT DELETED.id WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Operators não encontrado(a) para deletar.' });
        }
