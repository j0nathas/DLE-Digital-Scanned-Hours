...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const maquinas = ['s.SM06ty', 's.SM07st', 's.SM08st', 's.SM09st', 's.SM10st', 's.SM11st', 's.SM12ni', 's.SM13st'];
    ...
    async function atualizarEsperadoEColorirLinhas() {
        ...
        try {
            ...
            for (const maquina of maquinas) {
                ...
                const botaoProduto = document.querySelector(`[data-produto="${maquina}"]`);
    ...
    if (botaoConfirmarOperacao) {
        botaoConfirmarOperacao.addEventListener('click', async () => {
            ...
            try {
                ...
                const linha = botaoSelecionado.dataset.uap;
    ...
    function atualizarCorOperacoes() {
        botoesOperacao.forEach(botao => {
            const pElements = botao.querySelectorAll('.text-qntOperadores-uap');
            if (pElements.length >= 3) {
                ...
                botao.classList.remove('uap-verde', 'uap-vermelho');
                if (!isNaN(valAtual) && !isNaN(valEsperado)) {
                    if (valAtual === valEsperado) {
                        botao.classList.add('uap-verde');
                    } else if (valAtual > valEsperado) {
                        botao.classList.add('uap-vermelho');
                    }
                }
            }
        });
    ...
    function reiniciarInput() {
        inputPesquisa.forEach(input => {
            ...
            botoesProdutos.forEach(botao => {
                ...
                const cod_LE = (botao.getAttribute('data-cod-ld') || '').toUpperCase();
