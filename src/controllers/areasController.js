...
exports.getAllAreas = async (req, res) => {
    try {
        ...
        const result = await pool.request().query('SELECT * FROM dbo.Areas');
...
exports.getAreasById = async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query('SELECT * FROM dbo.Areas WHERE id = @id');

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Areas não encontrado(a).' });
        }
...
exports.createAreas = async (req, res) => {
    ...
    "maxOccupancy": "int"
        ...
        const query = `INSERT INTO dbo.Areas (${columns.join(', ')}) OUTPUT INSERTED.* VALUES (${variables.join(', ')});`;
...
exports.updateAreas = async (req, res) => {
    ...
    "maxOccupancy": "int"
        ...
        const query = `UPDATE dbo.Areas SET ${setClauses.join(', ')} OUTPUT INSERTED.* WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Areas não encontrado(a) para atualização.' });
        }
        res.status(200).json(result.recordset[0]);
...
exports.deleteAreas = async (req, res) => {
    try {
        ...
        const pool = await poolPromise();
        const request = pool.request();
        const query = `DELETE FROM dbo.Areas OUTPUT DELETED.id WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Areas não encontrado(a) para deletar.' });
        }
