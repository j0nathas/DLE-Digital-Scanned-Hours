...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const maquinas = ['s.LA01st', 's.LA02st', 's.LA03gm', 's.LA04gm'];

    const maquinasLinha = ['s.LA01st_s.LA02st', 's.LA03gm_s.LA04gm'];
    ...
    for (const linha of maquinasLinha) {
        verificarStatusDaLinha(linha).then(status => {
            if (status === 'Produzindo') {
                ...
                window.location.href = '/s.LA01_02_03_04/html/telaPrincipal.html';
            } else {
                console.log(`A linha ${linha} já está produzindo! Redirecionando...`);
            }
        });
