...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const maquinas = ['s.LA05gm', 's.LA06gm', 's.LA07re', 's.LA08re'];

    const maquinasLinha = ['s.LA05gm_s.LA06gm', 's.LA07re_s.LA08re'];
    ...
    for (const linha of maquinasLinha) {
        verificarStatusDaLinha(linha).then(status => {
            if (status === 'Produzindo') {
                ...
                window.location.href = '/s.LA05_06_07_08/html/telaPrincipal.html';
            } else {
                console.log(`A linha ${linha} já está produzindo! Redirecionando...`);
            }
        });
