...
exports.getGroupsById = async (req, res) => {
    try {
        ...
        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Groups não encontrado(a).' });
        }
...
exports.createGroups = async (req, res) => {
    ...
    "idType": "int"
        ...
        const query = `INSERT INTO dbo.Groups (${columns.join(', ')}) OUTPUT INSERTED.* VALUES (${variables.join(', ')});`;
...
exports.updateGroups = async (req, res) => {
    ...
    "idType": "int"
        ...
        const query = `UPDATE dbo.Groups SET ${setClauses.join(', ')} OUTPUT INSERTED.* WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Groups não encontrado(a) para atualização.' });
        }
        res.status(200).json(result.recordset[0]);
...
exports.deleteGroups = async (req, res) => {
    try {
        ...
        const pool = await poolPromise();
        const request = pool.request();
        const query = `DELETE FROM dbo.Groups OUTPUT DELETED.id WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Groups não encontrado(a) para deletar.' });
        }
