...
exports.getCardsById = async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query('SELECT * FROM dbo.Cards WHERE id = @id');

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Cards não encontrado(a).' });
        }
...
exports.createCards = async (req, res) => {
    ...
    "idVehicle": "bigint"
        ...
        const query = `INSERT INTO dbo.Cards (${columns.join(', ')}) OUTPUT INSERTED.* VALUES (${variables.join(', ')});`;
...
exports.updateCards = async (req, res) => {
    ...
    "idVehicle": "bigint"
        ...
        const query = `UPDATE dbo.Cards SET ${setClauses.join(', ')} OUTPUT INSERTED.* WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Cards não encontrado(a) para atualização.' });
        }
        res.status(200).json(result.recordset[0]);
...
exports.deleteCards = async (req, res) => {
    try {
        ...
        const pool = await poolPromise();
        const request = pool.request();
        const query = `DELETE FROM dbo.Cards OUTPUT DELETED.id WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Cards não encontrado(a) para deletar.' });
        }
