...
exports.getAllVehicles = async (req, res) => {
    try {
        ...
        const result = await pool.request().query('SELECT * FROM dbo.Vehicles');
...
exports.getVehiclesById = async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query('SELECT * FROM dbo.Vehicles WHERE id = @id');

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Vehicles não encontrado(a).' });
        }
...
exports.createVehicles = async (req, res) => {
    ...
    "color": "varchar"
        ...
        const query = `INSERT INTO dbo.Vehicles (${columns.join(', ')}) OUTPUT INSERTED.* VALUES (${variables.join(', ')});`;
...
exports.updateVehicles = async (req, res) => {
    ...
    "color": "varchar"
        ...
        const query = `UPDATE dbo.Vehicles SET ${setClauses.join(', ')} OUTPUT INSERTED.* WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Vehicles não encontrado(a) para atualização.' });
        }
        res.status(200).json(result.recordset[0]);
...
exports.deleteVehicles = async (req, res) => {
    try {
        ...
        const pool = await poolPromise();
        const request = pool.request();
        const query = `DELETE FROM dbo.Vehicles OUTPUT DELETED.id WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'Vehicles não encontrado(a) para deletar.' });
        }
