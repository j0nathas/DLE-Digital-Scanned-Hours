...
exports.getAllUserGroups = async (req, res) => {
    try {
        ...
        const result = await pool.request().query('SELECT * FROM dbo.UserGroups');
...
exports.getUserGroupsById = async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query('SELECT * FROM dbo.UserGroups WHERE id = @id');

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'UserGroups não encontrado(a).' });
        }
...
exports.createUserGroups = async (req, res) => {
    ...
    "isVisitor": "int"
        ...
        const query = `INSERT INTO dbo.UserGroups (${columns.join(', ')}) OUTPUT INSERTED.* VALUES (${variables.join(', ')});`;
...
exports.updateUserGroups = async (req, res) => {
    ...
    "isVisitor": "int"
        ...
        const query = `UPDATE dbo.UserGroups SET ${setClauses.join(', ')} OUTPUT INSERTED.* WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'UserGroups não encontrado(a) para atualização.' });
        }
        res.status(200).json(result.recordset[0]);
...
exports.deleteUserGroups = async (req, res) => {
    try {
        ...
        const pool = await poolPromise();
        const request = pool.request();
        const query = `DELETE FROM dbo.UserGroups OUTPUT DELETED.id WHERE id = @id;`;
        request.input('id', sql.BigInt, id);
        const result = await request.query(query);

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'UserGroups não encontrado(a) para deletar.' });
        }
